...
CodeAlpha Web Development Internship — Week 3 Assignment
Focus: DOM manipulation, JSON parsing, API integration
...
1. Open the `weather-app` folder in VS Code (`File > Open Folder...`)
...
- **`demo`** — uses [Open-Meteo](https://open-meteo.com), a free API that needs no signup. Good for instant local testing.
- **`openweather`** — uses [OpenWeatherMap](https://openweathermap.org/api), the API named in the assignment brief. Requires a free account and API key.
